  ...
  useEffect(() => {
    const loadTutorPosts = async () => {
      try {
        ...
        const approvedPosts = (res.data || []).filter((p: any) => p.status === 'Đang hiển thị').map((p: any) => ({
          id: p.id,
          title: p.title,
          isNew: true,
          tutorName: p.author_name ?? p.authorName ?? '',
          tutorInitials: (p.author_name || p.authorName || '').charAt(0).toUpperCase(),
          tutorId: p.author_id ?? p.authorId ?? 0,
          tutorRating: p.tutorRating ?? 0,
          subject: p.subject ?? '',
          price: p.price ?? '',
          format: p.format ?? '',
          schedule: p.schedule ?? p.time ?? '',
          description: p.description ?? p.desc ?? '',
          interested: p.applicants ?? 0,
          classType: p.classType ?? p.class_type ?? '',
          maxStudents: p.maxStudents ?? p.max_students ?? 0,
          registeredStudents: p.registeredStudents ?? p.registered_students ?? 0,
          postedAt: p.postedAt ?? '',
          color: p.color ?? ''
        }));
        setJobs(approvedPosts);
      } catch (err) {
        console.error('Failed to load tutor posts', err);
        setJobs([]);
      }
    };
